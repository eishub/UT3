...
public class ConnectingPanel extends JPanel {
	...
	public ConnectingPanel(Flag<URI> uriFlag, final IDisconnect disconnect) {
		this.connectionStatus = new JLabel("Connecting to: "
				+ uriFlag.getFlag());
		...
		uriFlag.addListener(new FlagListener<URI>() {
			...
			public void flagChanged(URI changedValue) {
				connectionStatus.setText("Connecting to: " + changedValue);
			}
		});
