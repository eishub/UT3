...
public class RunAgent {
	...
	public static void main(String[] args) throws EntityException,
		...
		UT3BotRunner<UT2004Bot, UT2004BotParameters> runner = new UT3BotRunner<UT2004Bot, UT2004BotParameters>(
				UT3BotBehavior.class, Configuration.DEFAULT_BOT_NAME,
				"127.0.0.1", 3000);
		...
		UT3BotBehavior behavior = ((UT3BotBehavior) agents.get(0)
				.getController());
		// targets.add(new NearestFriendlyWithLinkGun());
		// behavior.shoot(targets);
		// behavior.navigate(new
		// UnrealIdOrLocation(UnrealId.get("UTWeaponLocker_Content_0")));
		// behavior.navigate(new
		// UnrealIdOrLocation(UnrealId.get("UTWeaponPickupFactory_4")));
		// behavior.navigate(new
		// UnrealIdOrLocation(UnrealId.get("UTWeaponPickupFactory_0")));
		behavior.navigate(new UnrealIdOrLocation(UnrealId
				.get("UTDeployablePickupFactory_0")));
		// behavior.navigate(new
		// UnrealIdOrLocation(UnrealId.get("UTAmmo_BioRifle_Content_0")));
		// behavior.navigate(new
		// UnrealIdOrLocation(UnrealId.get("UTPickupFactory_UDamage_0")));
		// behavior.navigate(new
		// UnrealIdOrLocation(UnrealId.get("PathNode_21")));
		...
		while (true) {
			...
			/*
			 * for (PerceptHandler handler : handlers) { LinkedList<Percept>
			 * list = handler.getAllPercepts(); for(Percept p : list) {
			 * if(p.getName().equals("item")) System.out.println(p); } }
			 */

			/*
			 * for(NavPoint n :
			 * behavior.getWorldView().getAll(NavPoint.class).values()) {
			 * 
			 * if(ItemType.getItemType(n.getVolume()) ==
			 * UT3ItemType.SLOW_VOLUME) System.out.println(n); }
			 */
			// System.out.println("Ammo enforcer: " +
			// behavior.getWeaponry().getAmmo(UT3ItemType.ENFORCER));
			...
			if (!behavior.getNavigation().isNavigating()) {
				...
				if (finalDesination && !droppedDeployable) {
					...
					// behavior.navigate(new
					// UnrealIdOrLocation(UnrealId.get("UTCTFBlueFlagBase_0")));
					// behavior.navigate(new
					// UnrealIdOrLocation(UnrealId.get("UTPickupFactory_Berserk_0")));
			...
			/*
			 * for (UT3Weapon weap :
			 * behavior.getWeaponry().getWeapons().values()) {
			 * System.out.println(weap.getType() + " ammo " +
			 * behavior.getWeaponry().getAmmo(weap.getType()) + " (max: " +
			 * behavior.getWeaponry().getMaxAmmo(weap.getType()) + ")"); }
			 */

			/*
			 * if(behavior.getInfo().hasPowerUp()) {
			 * System.out.println("Has POWERUP: " +
			 * behavior.getInfo().getPowerUp()); System.out.println("Time: " +
			 * behavior.getInfo().getPowerUpTime()); }
			 */

			/*
			 * for (PerceptHandler handler : handlers) { LinkedList<Percept>
			 * list = handler.getAllPercepts(); for(Percept p : list) {
			 * if(p.getName().equals("status")) System.out.println(p); } }
			 */
			...
			ArrayList<UnrealId> list = behavior.getVisibility()
			 ...
			 * if(!behavior.getNavigation().isNavigating()){
			 * if(!behavior.getInfo
			 * ().isAtLocation(behavior.getGame().getFlagBase(0))){
			 * behavior.navigate(id); } else { behavior.navigate(id2); } }
			 */
		}
