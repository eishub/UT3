...
public class UT3BotBehavior extends UT3BotModuleController<UT2004Bot> implements MyAllPerceptsProvider {
	 ...
	 * @return
	 */
	@AsPercept(name = "deployable", filter = Type.ALWAYS)
	...
	public Percept navigation() {
		...
		return p;
		/*
		 * ILocated currentTarget = navigation.getCurrentTarget(); // We are
		 * going nowhere. if (currentTarget == null) { IWorldObject
		 * lastTargetObject = (IWorldObject) navigation.getLastTarget();
		 * 
		 * if (lastTargetObject == null) { return new
		 * Percept(navigation.getState().getFlag(), new None()); } else { return
		 * new Percept(navigation.getState().getFlag(),
		 * lastTargetObject.getId()); } }
		 * 
		 * // We are going some place that has an unrealid. if (currentTarget
		 * instanceof IWorldObject) { IWorldObject targetObject = (IWorldObject)
		 * navigation.getCurrentTarget(); return new
		 * Percept(navigation.getState().getFlag(), targetObject.getId()); }
		 * 
		 * // We are going to a location(x,y,z) return new
		 * Percept(navigation.getState().getFlag(),
